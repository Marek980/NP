/* 
*	NP
*	Created by: Marc Lord
*	Created on: 22th November, 2022
* gcc -o NP NP.c -Wall `pkg-config --cflags --libs gtk+-3.0`
* appify -name "NP" -icon ./np.png ./NP
*/
...
int main(int argc, char *argv[]) {
   ...
   gtk_window_set_title(GTK_WINDOW(app.toplevel), "NP");	/* Name on window */
...
static void text_edit_init_GUI(TextEditUI *app) {
   ...
   //app->vbox = gtk_vbox_new(FALSE, 0); this is invalid in GTK3
   app->vbox = gtk_box_new(GTK_ORIENTATION_VERTICAL, 0); //Create a vbox inside the main Window
...
static void text_edit_create_menus(TextEditUI *app) {
   ...
   gtk_widget_add_accelerator(file->new, "activate", group, GDK_KEY_N,
                              GDK_CONTROL_MASK, GTK_ACCEL_VISIBLE);
   gtk_widget_add_accelerator(file->open, "activate", group, GDK_KEY_O,
                              GDK_CONTROL_MASK, GTK_ACCEL_VISIBLE);
   gtk_widget_add_accelerator(file->save, "activate", group, GDK_KEY_S,
                              GDK_CONTROL_MASK, GTK_ACCEL_VISIBLE);
   gtk_widget_add_accelerator(file->close, "activate", group, GDK_KEY_W,
                              GDK_CONTROL_MASK, GTK_ACCEL_VISIBLE);
   gtk_widget_add_accelerator(file->quit, "activate", group, GDK_KEY_Q,
                              GDK_CONTROL_MASK, GTK_ACCEL_VISIBLE); 
   ...
   gtk_widget_add_accelerator(edit->cut, "activate", group, GDK_KEY_X,
                              GDK_CONTROL_MASK, GTK_ACCEL_VISIBLE);
   gtk_widget_add_accelerator(edit->copy, "activate", group, GDK_KEY_C,
                              GDK_CONTROL_MASK, GTK_ACCEL_VISIBLE);
   gtk_widget_add_accelerator(edit->paste, "activate", group, GDK_KEY_V,
                              GDK_CONTROL_MASK, GTK_ACCEL_VISIBLE);
...
static void text_edit_show_about_dialog(GtkMenuItem *menu_item, GtkWindow *parent_window) {
   const gchar *authors[] = { "Marc Lord", NULL };
 
   gtk_show_about_dialog(parent_window,
                         "program-name", "NP (Note Pad)",
                         "authors", authors,
